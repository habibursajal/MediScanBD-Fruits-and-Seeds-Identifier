# ==============================================================================
# ABLATION CONDITION A: Cross-Entropy Loss + No TTA (Multi-Seed Folder Save)
...
BACKBONE_LR, HEAD_LR, WEIGHT_DECAY = 1e-5, 3e-4, 1e-2
...
def run_seed(seed, full_ds, split):
    ...
    res_json_path = seed_dir / 'A_CE_NoTTA_result.json'
    pred_npz_path = seed_dir / 'A_CE_NoTTA_predictions.npz'
    report_csv_path = seed_dir / 'A_CE_NoTTA_classification_report.csv'
    best_path = seed_dir / 'A_CE_NoTTA.pth'
    ...
    if res_json_path.exists() and pred_npz_path.exists() and report_csv_path.exists():
        try:
            ...
            print(f"  [WARNING] Seed {seed} files corrupted. Re-running...")
    ...
    res = {'seed': seed, 'condition': 'A_CE_NoTTA', 'test_accuracy': float(acc), 'macro_f1': float(macro_f1)}
...
if __name__ == '__main__':
    ...
    print("=" * 80 + "\n[CONDITION A] Processing Seeds: " + str(SEEDS) + "\n" + "=" * 80)
    ...
    if all_results:
        summary_csv = OUT_DIR / 'A_CE_NoTTA_all_seeds_summary.csv'
