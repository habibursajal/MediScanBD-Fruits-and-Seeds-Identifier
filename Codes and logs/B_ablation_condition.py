# ==============================================================================
# ABLATION CONDITION B: Focal Loss + No TTA (Multi-Seed Folder Save)
# Output: Reviewer_Ablation_Statistics/seed_/
...
def run_seed(seed, full_ds, split):
    ...
    res_json_path = seed_dir / 'B_Focal_NoTTA_result.json'
    pred_npz_path = seed_dir / 'B_Focal_NoTTA_predictions.npz'
    report_csv_path = seed_dir / 'B_Focal_NoTTA_classification_report.csv'
    best_path = seed_dir / 'B_Focal_NoTTA.pth'
    
    # --------------------------------------------------------------------------
    # RESUME / SKIP LOGIC
    # --------------------------------------------------------------------------
    if res_json_path.exists() and pred_npz_path.exists() and report_csv_path.exists():
        try:
            res = json.loads(res_json_path.read_text())
            print(f"  [SKIP] Seed {seed} already completed! (Accuracy: {res.get('test_accuracy', 0)*100:.2f}%)")
            return res
        except Exception:
            print(f"  [WARNING] Seed {seed} result file corrupted. Re-running...")
    ...
    train_loader = DataLoader(MapDataset(full_ds, split['train_idx'], train_tf), batch_size=BATCH_SIZE, shuffle=True, **kwargs)
    ...
    res = {'seed': seed, 'condition': 'B_Focal_NoTTA', 'test_accuracy': float(acc), 'macro_f1': float(macro_f1)}
    ...
    # 1. Save Full Classification Report (Precision, Recall, F1 per class)
    ...
    # 2. Save JSON Result & Prediction Probabilities (NPZ)
...
if __name__ == '__main__':
    ...
    print("=" * 80 + "\n[CONDITION B] Processing Seeds: " + str(SEEDS) + "\n" + "=" * 80)
    ...
    if all_results:
        summary_csv = OUT_DIR / 'B_Focal_NoTTA_all_seeds_summary.csv'
