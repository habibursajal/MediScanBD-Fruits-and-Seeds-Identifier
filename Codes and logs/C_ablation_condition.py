# ==============================================================================
# ABLATION CONDITION C: Cross-Entropy Loss + 4-Pass TTA (Multi-Seed Folder Save)
...
HEAD_LR, WEIGHT_DECAY = 1e-4, 1e-2  # Adjusted Head LR for stable optimization
...
class TSHE(nn.Module):
    def __init__(self, seed):
        ...
        # Freeze Backbones
        for p in self.s1.parameters(): p.requires_grad = False
        for p in self.s2.parameters(): p.requires_grad = False
        for p in self.s3.parameters(): p.requires_grad = False

        # Enforce seed before ALL trainable heads
        torch.manual_seed(seed)
...
def run_seed(seed, full_ds, split):
    ...
    res_json_path = seed_dir / 'C_CE_4PassTTA_result.json'
    pred_npz_path = seed_dir / 'C_CE_4PassTTA_predictions.npz'
    report_csv_path = seed_dir / 'C_CE_4PassTTA_classification_report.csv'
    best_path = seed_dir / 'C_CE_4PassTTA.pth'
    ...
    if res_json_path.exists() and pred_npz_path.exists() and report_csv_path.exists() and best_path.exists():
        try:
            ...
            print(f"  [SKIP] Seed {seed} for Condition C already completed! (Accuracy: {res.get('test_accuracy', 0)*100:.2f}%)")
            return res
        except Exception:
    ...
    model = TSHE(seed).to(DEVICE)
    opt = optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=HEAD_LR, weight_decay=WEIGHT_DECAY)
    ...
    res = {'seed': seed, 'condition': 'C_CE_4PassTTA', 'test_accuracy': float(acc), 'macro_f1': float(macro_f1)}
...
if __name__ == '__main__':
    ...
    print("=" * 80 + "\n[CONDITION C] Processing Seeds: " + str(SEEDS) + "\n" + "=" * 80)
    ...
    if all_results:
        summary_csv = OUT_DIR / 'C_CE_4PassTTA_all_seeds_summary.csv'
